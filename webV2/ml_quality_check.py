...
parse_errors = {'H_vector': 0, 'DD_vector': 0, 'UD_vector': 0, 'UU_vector': 0, 'DU_vector': 0}
...
for vec_type, lengths in vector_lengths.items():
    if lengths:
        ...
        if unique_lengths == 1:
            print(f"✅ {vec_type}_vector: All {len(lengths)} samples have length {most_common_length}")
        else:
            print(f"⚠️ {vec_type}_vector: {unique_lengths} different lengths found!")
            print(f"   Distribution: {dict(length_distribution)}")
            print(f"   Most common: {most_common_length} ({length_distribution[most_common_length]} samples)")
