...
def migrate_data():
    ...
    with app.app_context():
        ...
        existing_users = int(
            db.session.execute(select(func.count()).select_from(User)).scalar_one()
        )
        ...
        if existing_users == 0 and user_count > 0:
            ...
            for user_row in users_data:
                ...
                new_user.created_at = (
                    datetime.fromisoformat(created_at) if created_at else datetime.utcnow()
                )
        ...
        if cursor.fetchone():
            ...
            existing_vectors = int(
                db.session.execute(select(func.count()).select_from(KeystrokeVector)).scalar_one()
            )
        ...
        if cursor.fetchone():
            ...
            existing_attempts = int(
                db.session.execute(select(func.count()).select_from(LoginAttempt)).scalar_one()
            )
