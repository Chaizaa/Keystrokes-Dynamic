...
for username in sorted(users_detail.keys()):
    ...
    print(
        f"{username:<20} | {enroll:2d}/10 ({enroll*10:3d}%) | {login:2d}/10 ({login*10:3d}%) | {status:^15}"
    )
...
complete_users = sum(
    1 for u, s in users_detail.items() if s["enrollment"] >= 10 and s["login"] >= 10
)
...
if len(collection_samples) > 0:
    ...
    print(
        f"   📊 Ratio: {len(collection_samples)}/{len(data)} ({len(collection_samples)/len(data)*100:.1f}%)"
    )
else:
