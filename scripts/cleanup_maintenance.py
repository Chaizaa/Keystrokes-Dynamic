...
def show_statistics():
    ...
    try:
        db_path = os.path.join(os.path.dirname(__file__), '..', 'biometric_auth.db')
        ...
        for table in tables:
            ...
            if table == 'verified_logins':
                # Count by username
                cursor.execute("SELECT username, COUNT(*) as cnt FROM verified_logins GROUP BY username ORDER BY cnt DESC LIMIT 5")
                ...
                # Recent logins
                cursor.execute("SELECT COUNT(*) FROM verified_logins WHERE timestamp >= datetime('now', '-24 hours')")
                recent_24h = cursor.fetchone()[0]
                print(f"    └─ Last 24h: {recent_24h} logins")
                
            elif table == 'failed_logins':
                # Count by reason
                cursor.execute("SELECT reason, COUNT(*) as cnt FROM failed_logins GROUP BY reason ORDER BY cnt DESC")
                ...
                # Recent failures
                cursor.execute("SELECT COUNT(*) FROM failed_logins WHERE timestamp >= datetime('now', '-24 hours')")
                recent_24h = cursor.fetchone()[0]
                print(f"    └─ Last 24h: {recent_24h} failures")
                
            elif table == 'enrollment_vectors':
                # Count by username
                cursor.execute("SELECT username, COUNT(*) as cnt FROM enrollment_vectors GROUP BY username ORDER BY cnt DESC LIMIT 5")
                top_users = cursor.fetchall()
                if top_users:
                ...
                # Latest stats
                cursor.execute("SELECT date, total_attempts, successful_logins, failed_logins FROM login_statistics ORDER BY date DESC LIMIT 3")
    ...
    try:
        ...
        if stats:
            ...
            for row in stats:
                ...
                print(f"   {date} | {total:>5} | {success:>7} | {failed:>6} | {avg_score_str:>9} | {users:>5}")
            print("   " + "-"*66)
...
def main():
    ...
    parser.add_argument(
        '--aggregate',
        action='store_true',
        help='Aggregate login statistics'
    )
