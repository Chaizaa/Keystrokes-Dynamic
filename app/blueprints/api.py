...
from app.utils.password_strength import calculate_password_strength, get_strength_label, get_strength_recommendations
...
def verify_email():
    ...
    try:
        ...
        if not username or not token:
            return jsonify({'success': False, 'message': 'Data tidak lengkap', 'error_code': 'invalid_input'}), 400
        ...
        if not user:
            return jsonify({'success': False, 'message': 'User tidak ditemukan', 'error_code': 'user_not_found'}), 404
        ...
        try:
            ...
            if sent_at:
                ...
                if datetime.now(timezone.utc) > (sent_at + timedelta(hours=int(expiry_hours))):
                    return jsonify({'success': False, 'message': 'Token kadaluarsa', 'error_code': 'expired_token'}), 400
        ...
        try:
            ok, reason = email_service.verify_token(token, user.email, user.email_verification_sent_at, code_hash=user.email_verification_code_hash)
        except TypeError:
            # Backwards-compatible fallback for tests or older monkeypatches that expect 3 args
            ok, reason = email_service.verify_token(token, user.email, user.email_verification_sent_at)
        if not ok:
            if reason == 'expired':
                return jsonify({'success': False, 'message': 'Token kadaluarsa', 'error_code': 'expired_token'}), 400
            return jsonify({'success': False, 'message': 'Token tidak valid', 'error_code': 'invalid_token'}), 400
        ...
        db.session.commit()
        return jsonify({'success': True, 'message': 'Email terverifikasi'}), 200
...
def resend_verification():
    ...
    try:
        ...
        username = data.get('username')
        if not username:
            return jsonify({'success': False, 'message': 'Username required', 'error_code': 'invalid_input'}), 400
        from sqlalchemy import select
        user = db.session.execute(select(User).where(User.username == username)).scalars().first()
        if not user:
            return jsonify({'success': False, 'message': 'User not found', 'error_code': 'user_not_found'}), 404
        if not user.email:
            return jsonify({'success': False, 'message': 'No email on account', 'error_code': 'no_email'}), 400
        try:
            sent_at = datetime.now(timezone.utc)
            user.email_verification_sent_at = sent_at
...
def check_username():
    ...
    try:
        ...
        if status_str == 'resumable' and availability.get('exists'):
            response_data['message'] = f"Resume registration: {enrollment_count}/20 samples collected"
            ...
            if real_pass:
                ...
                if enrollment_count == 0:
                    ...
                    if existing_user:
                        ...
                        has_password = bool(getattr(existing_user, 'password_hash', None) or getattr(existing_user, 'plain_password', None))
                        if has_password:
                            try:
                            ...
                            try:
                                ...
                                db.session.rollback()
                                print(f"[ERROR] Failed to set password for existing user {username}: {e}")
...
def login():
    ...
    try:
        ...
        try:
            ...
            typed_len = int(result.get('features', {}).get('char_count', 0) or len(result.get('features', {}).get('H_vector', [])))

            # Fetch enrollment templates to find matching-length templates
            templates = []
            try:
                templates = biometric_service.db.get_enrollment_samples(username) if biometric_service and getattr(biometric_service, 'db', None) else []
            except Exception:
                try:
            ...
            if matched_len and user and getattr(user, 'password_hash', None):
                if real_pass is None or not user.check_password(real_pass):
                    db_manager.log_failed_login(username, 'password_mismatch', ip_address, user_agent)
                    return jsonify({'success': False, 'message': 'Password salah', 'reason': 'PASSWORD_MISMATCH'}), 403
                ...
                if stored_hash and password_hash and stored_hash != password_hash:
                    ...
                    return jsonify({'success': False, 'message': 'Password salah', 'reason': 'hash_mismatch'}), 403
        except Exception as e:
        ...
        if enrollment_status.get('ready_for_login'):
            try:
                ...
                templates = []
                try:
                    rows = sqlalchemy_db.session.execute(select(EnrollmentVector).where(EnrollmentVector.username == username)).scalars().all()
                    for r in rows:
                        t = {}
                        try:
                ...
                if not templates:
                    try:
                        rows = sqlalchemy_db.session.execute(select(KeystrokeVector).where(KeystrokeVector.username == username, (KeystrokeVector.event_type == 'enrollment') | (KeystrokeVector.data_type == 'enrollment'))).scalars().all()
                        for r in rows:
                            t = {}
                            try:
                                t['H_vector'] = json.loads(r.H_vector) if r.H_vector else []
                ...
                if templates:
                    ...
                    try:
                        verification_result = biometric_service.verify_keystroke_sample(result['features'], templates)
                    except Exception:
                    ...
                    if not verification_result.get('verified'):
                        try:
                            ...
                            confidence_label = 'exact_match' if calibrated_confidence >= _v.EXACT_MATCH_THRESHOLD else ('high' if calibrated_confidence >= _v.HIGH_CONFIDENCE_THRESHOLD else ('medium' if calibrated_confidence >= _v.MEDIUM_CONFIDENCE_THRESHOLD else ('low' if calibrated_confidence >= _v.LOW_CONFIDENCE_THRESHOLD else 'failed')))
                            if verified:
                                verification_result = {'success': True, 'verified': True, 'score': float(round(calibrated_confidence, 4)), 'avg_score': float(round(_np.mean([eu_score, cos_score, statistical_score]),4)) if (eu_scores or cos_scores or stat_scores) else 0.0, 'confidence': confidence_label, 'templates_used': len(templates), 'message': 'Biometric verification successful'}
                        except Exception as e:
                    ...
                    if not verification_result.get('verified'):
                        try:
                            verification_result = biometric_service.verify_keystroke_sample(username, result['features'])
                        except Exception:
                            verification_result = {'success': False, 'verified': False, 'score': 0.0}
                else:
                    verification_result = biometric_service.verify_keystroke_sample(username, result['features'])
        ...
        if not verification_result.get('verified'):
            ...
            try:
                from flask import current_app
                current_app.logger.debug(f"[VERIFICATION FAILED] {username} -> {verification_result}")
            except Exception:
            ...
            try:
                if enrollment_status['ready_for_login']:
                    ...
                    if templates:
                        secondary_ver = biometric_service.verify_keystroke_sample(result['features'], templates)
                        if secondary_ver.get('verified'):
                            ...
                            try:
                                ...
                                template_Hs = [_np.array(t.get('H_vector', []), dtype=float) for t in templates if t.get('H_vector')]
                                if template_Hs and login_H:
                                    login_arr = _np.array(login_H, dtype=float)
                                    # truncate to minimum length
                                    min_len = min(login_arr.shape[0], *[a.shape[0] for a in template_Hs])
                                    login_arr = login_arr[:min_len]
                                    aligned = [_np.array(a[:min_len], dtype=float) for a in template_Hs]
                                    ...
                                    if fallback_score >= 0.5:
                                        verification_result = {'success': True, 'verified': True, 'score': fallback_score, 'confidence': 'fallback'}
                            except Exception as _e:
            ...
            try:
                conf_score = float(verification_result.get('score') or verification_result.get('confidence_score') or 0.0)
            except Exception:
                conf_score = 0.0
...
def reset_password():
    ...
    try:
        ...
        if current_password:
            success, message = auth_service.change_password(username, current_password, new_password)
        else:
