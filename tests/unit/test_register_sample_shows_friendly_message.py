...
migrationPattern = re.compile(
    r"alembic|schema|database schema|run alembic|no (?:such )?column|has no column",
    re.IGNORECASE,
)
...
def test_friendly_message_maps_migration_texts():
    assert (
        friendly_message(
            "Unable to create account: database schema out of date (run alembic upgrade)"
        )
        == "Registration temporarily unavailable. Please try again later."
    )
    assert (
        friendly_message("Table users has no column named email")
        == "Registration temporarily unavailable. Please try again later."
    )
