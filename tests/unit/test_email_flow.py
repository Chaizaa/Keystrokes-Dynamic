...
def test_register_sample_email_send_failure_does_not_fail(client, monkeypatch, db_session):
    ...
    monkeypatch.setattr(es.email_service, 'generate_token', lambda email, salt=None: 'test-token', raising=False)
...
def test_register_sample_email_send_success_records_timestamp_and_token(client, monkeypatch, db_session):
    ...
    import app.services.email_service as es
    monkeypatch.setattr(es.email_service, 'generate_token', lambda email, salt=None: 'ok-token', raising=False)

    # Simulate a successful send (no exception)
    monkeypatch.setattr(es.email_service, 'send_verification_email', lambda user, token: True, raising=False)
