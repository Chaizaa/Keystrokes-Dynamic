...
class TestKeystrokeVectorModel:
    ...
    def test_keystroke_vector_timestamps(self, db_session):
        ...
        vector = KeystrokeVector(user_id=user.id, username="testuser", event_type="enrollment")
        db_session.add(vector)
        db_session.commit()
    ...
    def test_keystroke_vector_event_types(self, db_session):
        ...
        enrollment = KeystrokeVector(user_id=user.id, username="testuser", event_type="enrollment")
        login_attempt = KeystrokeVector(
            user_id=user.id, username="testuser", event_type="login_attempt"
        )
...
class TestLoginAttemptModel:
    ...
    def test_login_attempt_timestamp_timezone(self, db_session):
        ...
        attempt = LoginAttempt(username="testuser", success=True)
        db_session.add(attempt)
        db_session.commit()
    ...
    def test_login_attempt_success_tracking(self, db_session):
        ...
        success_attempt = LoginAttempt(user_id=user.id, username="testuser", success=True)
        ...
        successful = int(
            db_session.execute(
                ...
                .where(LoginAttempt.username == "testuser", LoginAttempt.success == True)
            ).scalar_one()
        )
        ...
        failed = int(
            db_session.execute(
                select(func.count())
                .select_from(LoginAttempt)
                .where(LoginAttempt.username == "testuser", LoginAttempt.success == False)
            ).scalar_one()
        )
    ...
    def test_login_attempt_repr(self, db_session):
        ...
        success_attempt = LoginAttempt(username="testuser", success=True)
        failed_attempt = LoginAttempt(username="testuser", success=False)
...
class TestModelRelationships:
    ...
    def test_user_keystroke_vectors_relationship(self, db_session):
        ...
        vectors = (
            db_session.execute(select(KeystrokeVector).where(KeystrokeVector.user_id == user.id))
            .scalars()
            .all()
        )
    ...
    def test_user_login_attempts_relationship(self, db_session):
        ...
        for i in range(5):
            attempt = LoginAttempt(user_id=user.id, username="testuser", success=(i % 2 == 0))
            db_session.add(attempt)
        db_session.commit()

        # Query attempts by user_id
        attempts = (
            db_session.execute(select(LoginAttempt).where(LoginAttempt.user_id == user.id))
            .scalars()
            .all()
        )
        ...
        successful = int(
            db_session.execute(
                select(func.count())
                .select_from(LoginAttempt)
                .where(LoginAttempt.user_id == user.id, LoginAttempt.success == True)
            ).scalar_one()
        )
        assert successful == 3
