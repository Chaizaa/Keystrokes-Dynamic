...
def test_login_skips_rate_limit_in_dev_and_allows_relaxed_score(client, db_session, monkeypatch, app):
    ...
    monkeypatch.setattr(api_mod.db_manager, 'get_failed_login_count_recent', lambda u, minutes=15: 6)

    # Ensure enrollment present
    monkeypatch.setattr(api_mod.biometric_service, 'get_enrollment_status', lambda u: {'count': 12, 'enrolled': True, 'ready_for_login': True})

    # Make verification return slightly below threshold
    fake_verif = {'success': True, 'verified': False, 'score': 0.695, 'confidence_score': 0.695, 'avg_score': 0.69, 'confidence': 'low', 'templates_used': 5}
