...
def test_verify_invalid_token(client, monkeypatch, db_session):
    ...
    def fake_process(events, username_arg):
        return {'status': 'success', 'features': {'H_vector':[0.1], 'DD_vector':[0.1]}, 'real_password_string': 'Pass123!'}
    monkeypatch.setattr('app.blueprints.api.process_web_events', fake_process)
    monkeypatch.setattr('app.blueprints.api.assess_sample_quality', lambda f: {'quality_label':'good','quality_score':0.9})

    # Create user by first sample
    resp = client.post('/api/register_sample', json={'username': username, 'events':[{'type':'k','key':'a'}], 'email': email})
    assert resp.status_code == 200

    # Monkeypatch token generation & sending
    import app.services.email_service as es
    monkeypatch.setattr(es.email_service, 'generate_token', lambda e, salt=None: 'right-token', raising=False)
...
def test_verify_expired_token(client, monkeypatch, db_session):
    ...
    import app.services.email_service as es
    monkeypatch.setattr(es.email_service, 'generate_token', lambda e, salt=None: 'exp-token', raising=False)
    monkeypatch.setattr(es.email_service, 'send_verification_email', lambda u, t: True, raising=False)
...
def test_resend_updates_token(client, monkeypatch, db_session):
    ...
    def fake_process(events, username_arg):
        return {'status': 'success', 'features': {'H_vector':[0.2], 'DD_vector':[0.2]}, 'real_password_string': 'Pass123!'}
    monkeypatch.setattr('app.blueprints.api.process_web_events', fake_process)
    monkeypatch.setattr('app.blueprints.api.assess_sample_quality', lambda f: {'quality_label':'good','quality_score':0.9})
    ...
    import app.services.email_service as es
    monkeypatch.setattr(es.email_service, 'generate_token', lambda e, salt=None: 'first', raising=False)
    monkeypatch.setattr(es.email_service, 'send_verification_email', lambda u, t: True, raising=False)
    ...
    monkeypatch.setattr(es.email_service, 'generate_token', lambda e, salt=None, sent_at=None: 'second', raising=False)
    ...
    monkeypatch.setattr(es.email_service, 'verify_token', lambda token, email, sent_at: (True, None) if token == 'second' else (False, 'invalid'), raising=False)
