...
def test_migration_copies_rows(client, db_session):
    ...
    if not cols:
        cur.execute('''CREATE TABLE user_vectors (
    ...
    cur.execute("INSERT INTO user_vectors (username, user_id, event_type, data_type, H_vector, DD_vector, raw_events, is_successful, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", ('migrateuser', uid, 'enrollment', 'enrollment', '[0.1,0.2]', '[0.05,0.06]', '[{"key":"a"}]', 1, ts))
    conn.commit()
    ...
    ev = db.session.execute(db.select(EnrollmentVector).where(EnrollmentVector.username == 'migrateuser')).scalars().first()
