...
class Verifier:
    ...
    def _extract_timing_vectors(self, samples):
        ...
        for sample in samples:
            ...
            hold_times = self._parse_vector(sample.get("H_vector") or sample.get("hold_times", []))
        ...
        if len(set(vector_lengths)) > 1:
            ...
            consistent_vectors = [
                v for v, length in zip(vectors, vector_lengths) if length == most_common_length
            ]

            print("[WARNING] Inconsistent vector lengths detected!")
            print("  - Length distribution:", dict(length_counts))
            print(
                "  - Using "
                + str(len(consistent_vectors))
                + "/"
                + str(len(vectors))
                + " samples with length "
                + str(most_common_length)
            )
    ...
    def _remove_outliers_iqr(self, vectors):
        ...
        if len(clean_vectors) < min_samples_required:
            print(
                f"[IQR Warning] Would remove too many samples ({len(vectors)} → {len(clean_vectors)}), keeping original data"
            )
            return vectors

        if n_removed > 0:
            print(
                f"[IQR Outlier Removal] {len(vectors)} → {len(clean_vectors)} samples ({n_removed} outliers removed)"
            )
    ...
    def _remove_outliers_zscore(self, vectors, threshold=3.0):
        ...
        if n_removed > 0:
            print(
                f"[Z-Score Outlier Removal] {len(vectors)} → {len(clean_vectors)} samples ({n_removed} outliers removed)"
            )
    ...
    def _remove_outliers_iforest(self, vectors):
        ...
        try:
            ...
            if n_removed > 0:
                print(
                    f"[Isolation Forest] {len(vectors)} → {len(clean_vectors)} samples ({n_removed} anomalies removed)"
                )
    ...
    def _calculate_adaptive_threshold(self, Y_train, false_accept_rate=0.05):
        ...
        if len(Y_train) < 3:
            print(
                f"[Adaptive Threshold Warning] Only {len(Y_train)} samples, using fallback threshold"
            )
        ...
        if len(Y_train) < 20:
            ...
            print(
                f"[Adaptive Threshold] Small dataset ({len(Y_train)} samples), using lenient FAR: 10%"
            )
        ...
        if adaptive_threshold == 0.0:
            ...
            print(
                f"[Adaptive Threshold] Adjusted too-small threshold to {adaptive_threshold:.6f} (small dataset protection)"
            )

        print(
            f"[Adaptive Threshold] Calculated: {adaptive_threshold:.4f} (FAR target: {false_accept_rate*100:.1f}%)"
        )
    ...
    def _train_mahalanobis(self, Y_train):
        ...
        if self.use_robust_stats:
            ...
            if n_samples < n_features:
                print(
                    f"[Robust Stats] Dataset too small for MCD ({n_samples} samples < {n_features} features), using regular covariance"
                )
                ...
                try:
                    ...
                    print(f"[Robust Stats] MCD failed, using regular covariance: {exc}")
                    cov_matrix = np.cov(Y_train, rowvar=False)
        else:
            mean_vector = np.mean(Y_train, axis=0)
            cov_matrix = np.cov(Y_train, rowvar=False)
        ...
        try:
            ...
            print(f"[Mahalanobis] Covariance inversion failed, using identity fallback: {exc}")
    ...
    def verify_user(self, new_features, enrollment_samples):
        ...
        stored_hash = (
            enrollment_samples[0].get("password_hash")
            or enrollment_samples[0].get("password")
            or ""
        )
        ...
        result_msg = (
            ...
            if is_genuine
            else f"❌ Impostor detected (score: {normalized_score:.3f} > threshold: {self.threshold})"
        )
    ...
    def verify_user_comprehensive(self, new_features, enrollment_samples):
        ...
        # 1. Check if enrollment data exists
        if not enrollment_samples or len(enrollment_samples) == 0:
            return {"error": True, "msg": "❌ No enrollment data", "results": {}}
        ...
        if new_hash and stored_hash and new_hash != stored_hash:
            return {"error": True, "msg": "❌ Wrong password", "results": {}}
        ...
        if n_samples < 20:
            print(
                f"  [Mahalanobis                   ] ⚠️ Skipped (dataset too small: {n_samples} samples < 20)"
            )
            results["mahalanobis"] = {
        ...
        if n_samples < 20:
            print(
                f"  [Mahalanobis + Robust Stats    ] ⚠️ Skipped (dataset too small: {n_samples} samples < 20)"
            )
        ...
        training_quality = {
            ...
            "n_samples_after_zscore": int(
                len(self._remove_outliers_zscore(Y_train_original.copy()))
            ),
            "n_samples_after_iforest": int(
                len(self._remove_outliers_iforest(Y_train_original.copy()))
            ),
        }
        ...
        print(
            f"🎯 CONSENSUS: {consensus['accept_count']}/{consensus['total_count']} methods accept"
        )
        print("=" * 60 + "\n")
    ...
    def _verify_single_method(
        self, method, Y_train, Y_score, use_robust=False, outlier_method="none"
    ):
        ...
        if len(Y_train_clean) < 3:
            print(
                f"  [{method_label}] ⚠️ Too few samples after outlier removal, using original data"
            )
        ...
        adaptive_threshold = self._calculate_adaptive_threshold(
            Y_train_clean, false_accept_rate=0.05
        )
        ...
        result_msg = (
            ...
            if is_genuine
            else f"❌ Reject (score: {normalized_score:.3f} > {adaptive_threshold:.3f})"
        )
    ...
    def _calculate_consensus(self, results):
        ...
        valid_results = [
            r for r in results.values() if r is not None and not r.get("skipped", False)
        ]
        ...
        accept_count = sum(1 for r in valid_results if r["result"])
        ...
        return {
            ...
            "agreement_percentage": float(
                (max(accept_count, reject_count) / len(valid_results)) * 100
            ),
        }
